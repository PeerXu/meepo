...
func (t *WebrtcTransport) onSystemRequestMessage(in Message) {
	...
	case SYS_METHOD_PING:
		h = transport_core.WrapHandleFuncGenerics[PingRequest](t.onPing)
	case SYS_METHOD_NEW_CHANNEL:
		h = transport_core.WrapHandleFuncGenerics[NewChannelRequest](t.onNewChannel)
	case SYS_METHOD_ADD_PEER_CONNECTION:
		h = transport_core.WrapHandleFuncGenerics[AddPeerConnectionRequest](t.onAddPeerConnection)
	case SYS_METHOD_CLOSE:
		h = transport_core.WrapHandleFuncGenerics[CloseRequest](t.onClose)
	default:
		var ok bool
